...
export function downloadBrief({ form, sliders, portfolioProfile, chits, poiCount, selectedTargets = [] }) {
  ...
  const sections = chits.map((chit, index) => `<section class="target">
    ...
    <h3>TACTICAL IMPACT</h3><p>${esc(chit.tacticalImpact)}</p><p><b>Legal / Tactical Classification:</b> ${esc((chit.legalTacticalTypes || []).join(', '))}</p>
