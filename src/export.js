...
export function downloadBrief({ form, sliders, portfolioProfile, chits, poiCount, selectedTargets = [] }) {
  ...
  const sections = chits.map((chit, index) => `<section class="target">
    <h2>${esc(chit.tier || 'B')}${index + 1} — ${esc(chit.category || 'POI')} — ${esc(chit.target)}</h2>
    ...
    <h3>EVIDENCE</h3>${(chit.evidence || []).map((e, sourceIndex) => `<p><b>[${sourceIndex + 1}] <a href="${esc(e.url)}">${esc(e.organization || e.publication || 'Source')} — ${esc(e.title || e.source)}</a></b><br>${esc(e.date || 'VERIFICATION REQUIRED')} · ${esc(e.sourceClassification || 'VERIFICATION REQUIRED')} · ${esc(e.status || 'VERIFICATION REQUIRED')}<br><b>Claim:</b> ${esc(e.claim)}<br><b>Evidence:</b> “${esc(e.verbatimEvidence || 'No verbatim supporting evidence available.')}”</p>`).join('')}
    ...
    <h3>TACTICAL IMPACT</h3><p>${esc(chit.tacticalImpact)}</p><p><b>Tier:</b> ${esc(chit.tier || 'B')} · <b>Confidence:</b> ${Math.round((chit.confidence || 0) * 100)}%</p><p><b>Legal / Tactical Classification:</b> ${esc((chit.legalTacticalTypes || []).join(', '))}</p>
  ...
  </style></head><body><h1>CHITFORGE<br>TACTICAL POI BRIEF</h1><p><b>COMMITTEE:</b> ${esc(form.committee || 'Unspecified')}<br><b>AGENDA:</b> ${esc(form.agenda)}<br><b>PORTFOLIO:</b> ${esc(form.portfolio)}<br><b>TARGETS:</b> ${esc(targetSummary)}<br><b>NUMBER OF POIs:</b> ${chits.length}${poiCount ? ` / ${poiCount} requested` : ''}<br><b>DATE:</b> ${new Date().toLocaleString()}</p><h2>EXECUTIVE SUMMARY</h2><p>${esc(portfolioProfile?.summary || 'VERIFICATION REQUIRED')}</p><p><b>Interests:</b> ${esc((portfolioProfile?.interests || []).join('; '))}</p>${sections}<footer>ChitForge tactical brief · generated locally in browser</footer></body></html>`;
