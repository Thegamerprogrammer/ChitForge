...
async function rawGenerate(apiKey, model, prompt, schema, { timeoutMs = 70000, nativeJson = true } = {}) {
  ...
  try {
    const res = await fetch(endpoint(apiKey, model.id), { method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: controller.signal, body: JSON.stringify(buildBody(prompt, schema, model, { nativeJson })) });
  ...
  for (const model of [selected, ...ranked.filter((m) => m.id !== selected.id)]) {
    ...
    try {
      try { return { text: await rawGenerate(apiKey, model, prompt, schema, { timeoutMs, nativeJson: true }), model, mode: modelMode, fallbackLog, usedNativeJson: true }; }
        ...
        if (err.category === 'http-400') return { text: await rawGenerate(apiKey, model, prompt, schema, { timeoutMs, nativeJson: false }), model, mode: modelMode, fallbackLog, usedNativeJson: false };
        throw err;
      }
    } catch (err) {
