...
const endpoint = (key) => `${BASE_URL}/${API_VERSION}/models/${MODEL}:generateContent?key=${encodeURIComponent(key)}`;
...
function userMessageForStatus(status, reason) {
  ...
  if (status === 400) return 'Gemini rejected the request payload. The prompt or JSON configuration was invalid.';
  if (status === 401) return 'Invalid Gemini API key. Check your key and try again.';
  if (status === 403) return 'Gemini rejected the request. Check API access and model permissions.';
  if (status === 404) return 'Gemini model unavailable or not found. Try again shortly or check model access.';
...
function debugDiagnostic({ status, reason, category, apiKey }) {
  if (!import.meta.env.DEV) return undefined;
  return [`GEMINI ERROR`, `Model: ${MODEL}`, `Status: ${status || 'n/a'}`, `Category: ${category}`, `Reason: ${reason || 'n/a'}`, `API key: ${redact(apiKey)}`].join('\n');
}

export async function callGemini(apiKey, prompt, { timeoutMs = 70000 } = {}) {
  ...
  try {
    const res = await fetch(endpoint(apiKey), { method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: controller.signal, body: JSON.stringify(body) });
    if (!res.ok) {
      ...
      throw new GeminiError(userMessageForStatus(res.status, reason), { category, status: res.status, reason, diagnostic: debugDiagnostic({ status: res.status, reason, category, apiKey }) });
    }
    let data;
    ...
    if (error.name === 'AbortError') throw new GeminiError('Gemini request timed out. Try fewer POIs or try again shortly.', { category: 'timeout', cause: error });
